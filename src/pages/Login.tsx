        ...
        {!isLogin && (
          <div style={s.agree}>
            <Checkbox checked={agreed} onChange={setAgreed} style={{ '--icon-size':'16px', fontSize:12 } as any}>
              同意 <span style={{color:'var(--c-primary-deep)'}}>《用户协议》</span>和<span style={{color:'var(--c-primary-deep)'}}>《隐私政策》</span>
            </Checkbox>
          </div>
        )}
...
const s: Record<string, React.CSSProperties> = {
  ...
  title: { fontSize: 28, fontWeight: 500, color: 'var(--c-text)', marginBottom: 6, letterSpacing: 1 },
  sub: { fontSize: 14, color: 'var(--c-text-secondary)', fontWeight: 300 },
  form: { '--border-inner': 'none', '--border-top': 'none', '--border-bottom': '1px solid rgba(200,182,166,0.15)' } as any,
  input: {
    '--font-size': '15px',
    '--placeholder-color': '#C0BBB6',
    padding: '14px 0',
  } as any,
  codeBtn: { color: 'var(--c-primary-deep)', fontSize: 13, fontWeight: 500 },
  ...
  btn: {
    ...
    fontSize: 16,
    fontWeight: 500,
    boxShadow: '0 6px 24px rgba(200,182,166,0.35)',
    border: 'none',
    marginTop: 12,
  } as any,
  switch: { textAlign: 'center', marginTop: 28, fontSize: 14, color: 'var(--c-text-secondary)', fontWeight: 300 },
  switchLink: { color: 'var(--c-primary-deep)', fontWeight: 500, cursor: 'pointer', marginLeft: 4 },
}
